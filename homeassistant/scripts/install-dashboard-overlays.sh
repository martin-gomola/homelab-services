...
STORAGE_OVERVIEW_TEMPLATE="${REPO_DASHBOARD_DIR}/overview.storage.json"
STORAGE_OVERVIEW_INSTALLER="${PROJECT_DIR}/scripts/install-storage-overview.py"
STORAGE_DASHBOARD="${HA_CONFIG_DIR}/.storage/lovelace.lovelace"
...
if [ -f "${STORAGE_OVERVIEW_TEMPLATE}" ] && [ -f "${STORAGE_DASHBOARD}" ]; then
  python3 "${STORAGE_OVERVIEW_INSTALLER}" \
    "${STORAGE_OVERVIEW_TEMPLATE}" \
    "${STORAGE_DASHBOARD}"
fi
